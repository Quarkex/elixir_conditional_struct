defmodule Conditional do
  ...
  Known map operators are the logic gates `and`, `or`, `xand`, `xor`, `not`,
  `nor` and `xnor`, the comparison operators `eq`, `neq`, `gt`, `ge`, `lt`
  and `le`, the check operators `in` and `match`, the disambiguator `cond`, the
  scape hatch `fn`, and the utility operators `var`, `count`, `each` y `sum`.
  ...
  defp is_in?(container, content) when is_bitstring(container) and is_bitstring(content),
    do: String.contains?(container, content)

  defp is_in?(container, content) when is_map(container) and is_map(content),
    do: MapSet.subset?(MapSet.new(content), MapSet.new(container))

  defp is_in?(container, content) when is_list(container),
  ...
  defp apply_cond_switch(map, state) when is_map(map) do
    map
    |> Map.to_list()
  ...
  defp digest({:eq, list}, state, false) do
    ...
    Enum.reduce(list, &if(&2 && List.first(list) == &1, do: true, else: false))
  end
  ...
  """
  def from_string("version:0.1.0;" <> data64),
  ...
  defimpl String.Chars, for: __MODULE__ do
    ...
    def to_string(instance),
      do: "version:#{@current_struct_version};" <> Base.encode64(:erlang.term_to_binary(instance))
  end
end
